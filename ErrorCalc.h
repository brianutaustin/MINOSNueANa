...
class ErrorCalc{
  ...
  protected:
    ...
    virtual void CalculateSystErrorMatrixGrid();//function to use for FC when using grid files instead of oscillating on the fly
    ...
    vector<int> FNExtrap_Flag;//0 for ONLY nc,numucc,bnue; 1 for numu extrapolation contribution to tau/signal, 2 for overall MC change to tau/signal
    ...
    vector<int> SpecialSyst_Flag;//0 for all components, 1 for NC+NuMuCC+BNueCC only, 2 for NuTauCC only, 3 for NueCC only, 4 for NuMuCC+NC (fake HOO error)
    
    vector<string> ExtraCovariance_SystName;
    vector<string> ExtraCovariance_File;
    vector<string> ExtraCovariance_HistName;
    vector<int> ExtraCovariance_Flag;//0 for all components, 1 for NC+NuMuCC+BNueCC only, 2 for NuTauCC only, 3 for NueCC only, 4 for NuMuCC+NC (fake HOO error), 5 for NC only, 6 for NuMuCC only, 7 for BNueCC only
    ...
    std::map<string,vector<TH2D*> > FN_BNueCC_Plus1Sigma;//if using f/n method for beam nues
